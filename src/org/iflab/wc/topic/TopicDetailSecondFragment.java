...
public class TopicDetailSecondFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.draft, container, false);
		 ...
		 * intent = new Intent(getActivity(),Answer.class);
		 * intent.putExtra("answerid",
		 * String.valueOf(datas.get(position).getAnswer_id()));
		...
		client.get(url, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				try {
					...
					if (total_rows == 0) {
						// Toast.makeText(getActivity(), "û����ѻش�",
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							String question_info = jsonObject2
									.getString("question_info");
							JSONObject question_infoJsonObject = new JSONObject(
									question_info);
							int question_id = question_infoJsonObject
									.getInt("question_id");
							bestAnswerModel.setQuestion_id(question_id);
							String question_content = question_infoJsonObject
									.getString("question_content");
							bestAnswerModel
									.setQuestion_content(question_content);
							String answer_info = jsonObject2
									.getString("answer_info");
							JSONObject answer_infoJsonObject = new JSONObject(
									answer_info);
							int answer_id = answer_infoJsonObject
									.getInt("answer_id");
							bestAnswerModel.setAnswer_id(answer_id);
							String answer_content = answer_infoJsonObject
									.getString("answer_content");
							bestAnswerModel.setAnswer_content(answer_content);
							int agree_count = answer_infoJsonObject
									.getInt("agree_count");
							bestAnswerModel.setAgree_count(agree_count);
							int uid = answer_infoJsonObject.getInt("uid");
							bestAnswerModel.setUid(uid);
							String avatar_file = answer_infoJsonObject
									.getString("avatar_file");
							bestAnswerModel.setAvatar_file(avatar_file);
							datas.add(bestAnswerModel);
						}
						TopicDetailSecondAdapter adapter = new TopicDetailSecondAdapter(
								getActivity(), datas);
